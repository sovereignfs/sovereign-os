...
- [RFC-0019: Home Assistant allowlisted-action capability mapping](0019-home-assistant-allowlisted-action-capability-mapping.md) - Accepted
...
the first mutating capability. Accepted the same day after review found
and fixed a real gap: domain scope was claimed to be enforced at three
points but only one was actually designed — closed by adding two
independent domain re-checks, since Home Assistant's `climate` domain
turns out to have real `turn_on`/`turn_off` services that wouldn't have
failed safely by accident.
